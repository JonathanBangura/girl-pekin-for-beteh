...
export async function ApplicationsManagementPage({
      ...
      <div className="portal-content v2-admin-page">
        <div className="v2-admin-page-head">
          <div>
            ...
            <h1>Applications</h1>
            <p>
              No award edition is available to your review role.
            </p>
          </div>
        </div>
      </div>
  ...
  const messages: Record<string, string> = {
    missing_fields:
      'The application action is incomplete.',
    not_found:
      'The nominee application could not be found.',
    invalid_transition:
      'That workflow transition is not allowed from the current state.',
    reject_note_required:
      'Enter a rejection reason before rejecting the application.',
    transition_failed:
    ...
    <div className="portal-content v2-admin-page mobile-admin-page">
      <div className="v2-admin-page-head">
        <div>
          ...
          <p>
            Review public and staff-created nominee applications
            before they become approved nominees. Approval does
            not publish a nominee automatically.
          </p>
        </div>
      ...
      >
        <label>
          ...
          <select
            name="edition"
            defaultValue={data.selectedEdition.id}
          >
      ...
      <div className="v2-admin-stats mobile-admin-stats">
        <article>
          <span>Submitted</span>
          <strong>{data.counts.submitted}</strong>
          <small>Awaiting review</small>
        </article>

        <article>
          <span>Under review</span>
          <strong>{data.counts.under_review}</strong>
        ...
        <article>
          ...
          <small>
            Ready for nominee publication workflow
          </small>
        </article>
      ...
      <section className="panel v2-admin-table-panel">
        <div className="v2-admin-table-toolbar">
          <div>
            <h2>
              {data.selectedEdition.edition_label} ·{' '}
              {data.selectedEdition.year}
            </h2>
                    ...
                    <td>
                      {detail
                        ? detail.application_type ===
                          'self'
                          ? 'Public self-application'
                          : 'Public nomination'
                        : 'Staff / existing record'}
                    </td>
                    ...
                    <td>
                      <details className="live-row-editor wide mobile-row-editor">
                        ...
                        <div className="live-management-stack">
                          <div>
                            <strong>
                              Biography / Application Information
                            </strong>
                            <p className="live-empty-copy">
                              {application.bio ||
                                'No biography/application summary has been entered.'}
                            </p>
                          </div>
                            ...
                            <>
                              <div>
                                ...
                                <p className="live-empty-copy">
                                  Nominee contact:{' '}
                                  {contactLine(
                                    detail.nominee_email,
                                    detail.nominee_phone,
                                  )}
                                </p>
                              </div>
                              ...
                              'nomination' ? (
                                <div>
                                  ...
                                  <p className="live-empty-copy">
                                    {detail.nominator_name ||
                                      '—'}
                                    {detail.nominator_relationship
                                      ? ` · ${detail.nominator_relationship}`
                                      : ''}
                                  </p>
                                  <p className="live-empty-copy">
                                    {contactLine(
                                      detail.nominator_email,
                                      detail.nominator_phone,
                                    )}
                                  </p>
                                </div>
                              ) : null}
      ...
      <section className="panel">
        ...
        <p className="live-empty-copy">
          Applications move through Submitted → Under Review →
          Approved/Rejected. Approved records continue to the
          Nominees workspace, where publication and nominee-portal
          linking are handled separately.
        </p>
      </section>
    </div>
  )
