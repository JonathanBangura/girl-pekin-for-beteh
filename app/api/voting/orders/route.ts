...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return NextResponse.json(
      { error: 'Please provide a valid nominee, quantity and contact details.' },
      { status: 400 },
    )
  }
  ...
  const { data: nominee } = await admin
    .from('nominees')
    .select('id,award_edition_id,nominee_code,full_name,status,is_public')
    .eq('nominee_code', nomineeCode)
    .maybeSingle()

  if (!nominee || nominee.status !== 'published' || nominee.is_public !== true) {
    return NextResponse.json(
  ...
  const [{ data: edition }, { data: pricing }] = await Promise.all([
    admin
      .from('award_editions')
      .select('id,year,status,is_public,voting_starts_at,voting_ends_at')
      .eq('id', nominee.award_edition_id)
      .maybeSingle(),
    admin
      .from('vote_pricing')
      .select('unit_price,currency,min_quantity,max_quantity,is_active')
      .eq('award_edition_id', nominee.award_edition_id)
      .maybeSingle(),
  ])

  if (!edition || edition.is_public !== true || edition.status !== 'voting_open') {
    return NextResponse.json({ error: 'Voting is currently closed.' }, { status: 409 })
  }
  ...
  if (!pricing?.is_active) {
    return NextResponse.json({ error: 'Vote pricing is not active.' }, { status: 409 })
  }

  if (quantity < pricing.min_quantity || quantity > pricing.max_quantity) {
    return NextResponse.json(
  ...
  const { data: order, error: orderError } = await admin
    ...
    .select('id,order_number,public_token,total_amount,currency')
    .single()

  if (orderError || !order) {
    console.error('create vote order', orderError)
    return NextResponse.json({ error: 'Unable to create the vote order.' }, { status: 500 })
  }
  ...
  const { data: payment, error: paymentError } = await admin
    ...
    .insert({
      ...
      provider_payload: { integration_status: 'adapter_pending' },
    })
    .select('id')
    .single()
  ...
  if (paymentError || !payment) {
    ...
    await admin.from('vote_orders').update({ status: 'cancelled' }).eq('id', order.id)
  ...
  return NextResponse.json(
    {
      ...
      status_url: `/vote/order/${order.public_token}`,
      message:
        'Vote order created. Vult payment initiation will be enabled after the exact provider API contract is connected.',
    },
    { status: 201 },
  )
}
