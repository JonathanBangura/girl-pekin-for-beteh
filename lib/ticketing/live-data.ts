...
export async function getPublicTicketCheckoutOffer({
  ...
  const now = Date.now()
  const starts =
    ticketType.sales_starts_at
      ? new Date(
          ticketType.sales_starts_at,
        ).getTime()
      : null
  const ends =
    ticketType.sales_ends_at
      ? new Date(
          ticketType.sales_ends_at,
        ).getTime()
      : null
...
export async function getPublicTicketOrderStatus(
  token: string,
) {
  const supabase = createPublicClient()
...
export async function getAdminEventsData() {
  ...
  const [
    ...
    supabase
      .from('events')
      .select(
        'id,award_edition_id,slug,title,summary,description,venue,starts_at,ends_at,access_type,status,is_public,capacity,cover_image_url,updated_at',
      )
      .order('starts_at', {
        ascending: false,
      }),
  ...
  const [
    ...
    orderIds.length
      ? supabase
          .from('payments')
          .select(
            'ticket_order_id,status,provider_transaction_id,created_at',
          )
          .in(
            'ticket_order_id',
            orderIds,
